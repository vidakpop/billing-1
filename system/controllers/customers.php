...
switch ($action) {
    ...
    case 'edit-post':
        ...
        break;
    case 'hotspot':
        run_hook('list_hotspot_customers'); #HOOK
        $search = _post('search');
        ...
        $query = ORM::for_table('tbl_customers')->where('service_type', 'Hotspot');
        if ($search != '') {
            $query->whereRaw("username LIKE '%$search%' OR fullname LIKE '%$search%' OR address LIKE '%$search%' " .
        ...
        if (_post('export', '') == 'csv') {
            ...
            header('Cache-Control: must-revalidate, post-check=0, pre-check=0');
            header("Content-type: text/csv");
            header('Content-Disposition: attachment;filename="phpnuxbill_customers_hotspot_' . $filter . '_' . date('Y-m-d_H_i') . '.csv"');
            header('Content-Transfer-Encoding: binary');
        ...
        $ui->assign('customers', $d);
        $ui->display('hotspot_customers.tpl');
        break;

    case 'pppoe':
        run_hook('list_pppoe_customers'); #HOOK
        $search = _post('search');
        $order = _post('order', 'username');
        $filter = _post('filter', 'Active');
        $orderby = _post('orderby', 'asc');
        ...
        $query = ORM::for_table('tbl_customers')->where('service_type', 'PPPOE');
        if ($search != '') {
            $query->whereRaw("username LIKE '%$search%' OR fullname LIKE '%$search%' OR address LIKE '%$search%' " .
                "OR phonenumber LIKE '%$search%' OR email LIKE '%$search%' AND status='$filter'");
        } else {
        ...
        if (_post('export', '') == 'csv') {
            ...
            header('Cache-Control: must-revalidate, post-check=0, pre-check=0');
            header("Content-type: text/csv");
            header('Content-Disposition: attachment;filename="phpnuxbill_customers_pppoe_' . $filter . '_' . date('Y-m-d_H_i') . '.csv"');
            header('Content-Transfer-Encoding: binary');
        ...
        $ui->assign('customers', $d);
        $ui->display('pppoe_customers.tpl');
        break;
